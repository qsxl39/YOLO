...
<template>
    <h1  class="text-3xl text-white font-bold flex items-center tracking-wide">
        YOLO算法盒子</h1>
    <div class="flex w-1/4 flex-wrap mx-auto justify-center">
        <div class="flex flex-col w-full text-xl">
            ...
            <input v-model="loginData.username" class="w-full border-b-gray-600 border-b pb-1" placeholder="请输入账户名" />
        </div>
        <div class="flex flex-col w-full text-xl">
            <label class="my-4 font-bold text-xl">
                密码
            </label>
            <input v-model="loginData.password" class="w-full border-b-gray-600 border-b pb-1" placeholder="请输入密码" />
        </div>
        <span class="w-full text-center tracking-wide mt-4 text-gray-600">忘记密码请联系管理员</span>
        <el-button @click="login" color="#000" size="large" class="btn mt-8">
            确定
        </el-button>
    </div>
</template>
